...
function renderFedcTarget(target) {
  ...
  target.predefinedActivities.forEach((pa, idx) => {
    ...
    const actData    = findActivityByPa(target.name, pa.name, pa.id);
...
function attachTargetListeners(target) {
  ...
  c.querySelectorAll(".btn-add-remark").forEach(btn => {
    ...
    btn.addEventListener("click", async () => {
      ...
      try {
        ...
        const paId    = btn.dataset.paId   || null;
        const paOrder = Number(btn.dataset.paOrder) || 0;
        let   actId   = btn.dataset.actId  || null;
        state.pendingNewActivity = null;
        if (paName) actId = await ensureFedcActivity(target.name, paName, paOrder, paId);
        if (!actId) { btn.disabled = false; btn.textContent = "+ Add Remark & Trials"; return; }
        let initialText = "";
        if (paName) {
          const pa = paId
            ? target.predefinedActivities?.find(a => a.id === paId)
            : target.predefinedActivities?.find(a => a.name === paName);
          if (pa?.isMastery) {
  ...
  c.querySelectorAll(".btn-init-predef-remark").forEach(btn => {
    btn.addEventListener("click", async () => {
      ...
      const actId = await ensureFedcActivity(tgt.name, btn.dataset.paName, paOrder, btn.dataset.paId || null);
  ...
  for (const target of (student.targets || [])) {
    for (const pa of (target.predefinedActivities || [])) {
      ...
      const existingAct = Object.entries(data.activities || {})
        .find(([, a]) => a.targetName === target.name && matchesPa(a, pa));
      ...
      if (!actId) {
        actId = await addActivity(sessionId, target.name, pa.name, pa.order ?? 0, true, pa.id || null);
      }
...
async function ensureFedcActivity(targetName, activityName, order, paId = null) {
  const existing = findActivityByPa(targetName, activityName, paId);
  if (existing) return existing.id;
  return await addActivity(state.currentSessionId, targetName, activityName, order, true, paId || null);
}
...
function viewActivityRows(no, actName, actId, data, target, isPredefined = true) {
  ...
  const paEntry = isPredefined
    ? (actConfigId
        ? target.predefinedActivities?.find(pa => pa.id === actConfigId)
    ...
    host.querySelectorAll(".predef-remark-input[data-pa-name]").forEach(el => {
      ...
      const create = async () => {
        try {
          ...
          const actId = await ensureFedcActivity(el.dataset.target, el.dataset.paName, paOrder, el.dataset.paId || null);
...
async function autoFillGroupSession(group, sessionId, data, targetName, attendees) {
  ...
  for (const pa of predefined) {
    const hasActivity = Object.values(data.activities || {})
      .some(a => a.targetName === targetName && matchesPa(a, pa));
    if (!hasActivity) {
      await addActivity(sessionId, targetName, pa.name, Date.now(), true, pa.id || null);
      created++;
    }
  }
  return created;
...
function buildGroupItemsByActivity(target, data, attendees) {
  ...
  for (const pa of (target.predefinedActivities || [])) {
    ...
    const actId = Object.entries(data.activities || {})
      .find(([, a]) => a.targetName === target.name && matchesPa(a, pa))?.[0] || null;
...
function findActivityByPa(targetName, paName, paId = null) {
  const found = Object.entries(state.sessionData?.activities || {}).find(([, a]) => {
    if (a.targetName !== targetName) return false;
    if (paId && a.configId) return a.configId === paId;
    return a.activityName === paName;
  });
  return found ? { id: found[0], ...found[1] } : null;
}
