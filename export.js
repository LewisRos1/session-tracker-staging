...
function addTrendSummarySheet(wb, allTargets, sessions) {
  ...
  for (const key of halfYearKeys) {
    ...
    // Overview chart — descending delta (best at top) to match AI report
    const chartRows = [...trendRows.filter(r => !r.noData)].sort((a, b) => b.delta - a.delta);
...
function drawOverviewChart(chartTrendRows, title) {
  ...
  const LEG_PAD = 22;
  const PAD_BTM = LEG_PAD + BOX + LEG_PAD;
...
// "Mastered" and "Discontinued" are generated by the app from each activity's
// status, unlike the section headings the user writes themselves. Colour says
// which of the two a reader is looking at without needing a legend.
// Discontinued is amber, never red: discontinuing is internal housekeeping and
// says nothing about the student, so a red band would read to a parent as a
// failure that the report deliberately never claims.
const sectionBandFill = label =>
  label === "Mastered" ? "#dcfce7" : label === "Discontinued" ? "#fef3c7" : "#e5e7eb";
const sectionBandText = label =>
  label === "Mastered" ? "#166534" : label === "Discontinued" ? "#92400e" : "#374151";
...
export function renderActivityBreakdownChart(targetName, activityData, periodLabel, chartTitle = null, showDuration = false, legendLabels = null) {
  ...
  for (const act of activityData) {
    if (act.isSectionHeader) { ctx.fillStyle = sectionBandFill(act.label); ctx.fillRect(0, yPos, W, SECTION_H); yPos += SECTION_H; }
  ...
  for (const act of activityData) {
    if (act.isSectionHeader) {
      ctx.font = "bold 14px sans-serif"; ctx.fillStyle = sectionBandText(act.label); ctx.textAlign = "left";
