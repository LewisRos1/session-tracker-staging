...
export async function addActivity(sessionId, targetName, activityName, order, isPredefined = false, configId = null) {
  const actId = generateId("a");
  const fields = { targetName, activityName, order, isPredefined };
  if (configId) fields.configId = configId;
  await updateDoc(doc(db, "sessions", sessionId), {
    [`activities.${actId}`]: fields
  });
  return actId;
}
